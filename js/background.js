 ...
 * (c) 2023 George Francis. All rights reserved.
 */

// Dependencies loaded as globals via <script> tags:
//   window.PIXI                        (pixi.min.js)
//   window.PIXI.filters.KawaseBlurFilter (pixi-filter-kawase-blur.js)
//   window.SimplexNoise                (simplex-noise.js)

const KawaseBlurFilter = PIXI.filters.KawaseBlurFilter;
...
function hsl(h, s, l) {
    ...
    const f = n => {
        const c = k(n) - 3;
        const v = l - a * Math.max(-1, Math.min(c, Math.min(9 - c, 1)));
